...
The app is designed to stay simple to install and operate. Runtime usage and encrypted configuration are stored in the local SQLite database. API keys are entered through the Config page and are encrypted with a machine-local key from `.env`.
...
- OpenAI-compatible LLM calling through:
  - OpenRouter
  - NVIDIA NIM
- Browser-based API key configuration with masked password fields and encrypted SQLite storage.
...
- Automatic `.env` encryption key generation for local secret storage.
...
Use this when editing `server.js`.

For the Vite React frontend, run the backend and frontend dev servers in two terminals:
...
Production frontend assets are built with:

```bash
npm run build
```

`npm start` serves `frontend/dist` when it exists and falls back to `public/` during migration.
...
CONFIG_ENCRYPTION_KEY=<auto-generated>
CONFIG_ADMIN_TOKEN=<optional remote admin token>
```

- `PORT`: local server port.
- `LLM_TIMEOUT_MS`: timeout per LLM fallback call in milliseconds.
- `CONFIG_ENCRYPTION_KEY`: generated automatically when missing. Keep `.env` with the SQLite DB; if `.env` is lost, saved API keys cannot be decrypted.
- `CONFIG_ADMIN_TOKEN`: optional token for protected config routes when the app is intentionally accessed from outside localhost.
