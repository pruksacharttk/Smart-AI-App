...
**Ask the user about (ONLY these — very rare):**
- Security vulnerabilities with multiple valid mitigation approaches
- Business logic ambiguity where the "correct" behavior depends on domain knowledge
- Architectural decisions that fundamentally change the approach (e.g., sync vs async)

**Just fix (don't ask — this should be 90%+ of items):**
...
- Missing tests
- **Technical decisions with a clear "best practice" answer** — pick it and note the decision
...
**The goal is autonomous operation.** Auto-fix everything that has an objectively better answer. Only escalate to user when there's genuine ambiguity that requires domain knowledge. When in doubt, **fix it yourself** rather than asking.
